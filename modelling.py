...
def generate_equations_pendulum_friction():
    # parameters
    m, I, l, g, c = symbols('m I l g c')
    # inputs
    u = symbols('u')
    # states
    theta, x, dtheta, dx = symbols('theta x theta_dot x_dot')
    ddx, ddtheta = symbols('ddx ddθ')
    dxcm = dx + l*(sp.cos(theta)*dtheta)
    dycm = l*(sp.sin(theta)*dtheta)
    ddxcm = ddx + l*(sp.cos(theta)*ddtheta - dtheta**2*sp.sin(theta))
    ddycm = l*(sp.sin(theta)*ddtheta + dtheta**2*sp.cos(theta))
    tau = c*dtheta
    A = -tau*sp.cos(theta) + m*ddxcm #-m*ddx
    N = m*ddycm + m*g-tau*sp.sin(theta)
    eq1 = m*ddx - u #A-tau*sp.cos(theta)-m*ddxcm
    eq2 = I*ddtheta - l*N*sp.sin(theta) - l*A*sp.cos(theta) 
    #N = (I*ddtheta - u*l*sp.cos(theta))/(l*sp.sin(theta))
    #eq2 = tau*sp.sin(theta)-m*g+N - m*ddycm
    #dE = m*dxcm*ddxcm + m*g*dycm + I*dtheta**2*ddtheta
    #fric = -c*dtheta**2 #-c*(ddtheta*l*theta + l*dtheta**2)
    #eq2 = dE + fric + A*dxcm
...
def generate_equations_pendulum_friction_file():
    # parameters
    f1, f2 = generate_equations_pendulum_friction()
    sep = '\n    '
    value = f'dx = x_dot{sep}dtheta = theta_dot{sep}'
    value += f'dx_dot = {f1}{sep}dtheta_dot = {f2}'
    template = ''
    with open('models/template_friction.txt', 'r') as f:
        template = f.read()
    func = template.replace('@@', value)
    with open('models/pendulum_friction.py', 'w') as f:
        f.write(func)
    print('new pendulum model generated')
#generate_equations_pendulum_friction_file()
...
def linearize_pendulum_friction(vars_val, par_val = [0.075, 0.07, 0.000001, 9.78, -0.0035]):
    x, x_dot, theta, theta_dot = symbols('x x_dot theta theta_dot')
    vars = [x, x_dot, theta, theta_dot]
    u = symbols('u')
    f1, f2 = generate_equations_pendulum_friction()
...
def simulate_friction(anim=False, u=lambda t, x : 0.0 , var0=[0.0, 0.0, np.pi*0.98, 0.0], t_start=0, t_end=5, lim=3, save=False):
    # Parametros
    m = 0.2 # kg
    l = 0.3 # m
    I = 0.006 # kg m²
    ...
    sol = odeint(pendulum_friction, var0, t, args=(u, par))
    ...
    if not anim:
        ...
        cm_car, = ax.plot([], [], 's-', markersize=10)
        rod, = ax.plot([], [], 'o-', lw=2, markersize=m/l)
        trajectory, = ax.plot([], [], 'r', lw=0.1, linestyle='--')
        def init():
            cm_car.set_data([], [])
...
#simulate_friction(anim=False, u=lambda t, x : 0.0 , var0=[0.0, 0.0, np.pi*0.9, 0.0], t_start=0, t_end=5, lim=3, save=False)