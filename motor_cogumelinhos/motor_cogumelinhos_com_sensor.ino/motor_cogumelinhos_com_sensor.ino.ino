...
void setup(void) {
  ...
  xTaskCreatePinnedToCore(MeasuringAngles,"Measuring angles",10000,NULL,1,&Task1,0);
...
void loop() {
  //if(meuStepper.distanceToGo() == 0 && aux == 0){
    //Serial.println("Acabou de se movimentar!!!!!!!!");
    //aux = 1;
  //}
  if(meuStepper.distanceToGo() == 0 && aux < 1) {
    aux++;
    passo = -1*passo;
    meuStepper.moveTo(passo);
  }
  meuStepper.run();
}